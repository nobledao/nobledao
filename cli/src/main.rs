...
fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            SubCommand::with_name("create-house")
                ...
                .arg(
                    Arg::with_name("coat_of_arms")
                        ...
                        .default_value("https://upload.wikimedia.org/wikipedia/commons/c/cd/Chief_demo.svg")
    ...
    match (sub_command, sub_matches) {
        ("show-house", Some(arg_matches)) => {
            let user_address = pubkey_of(arg_matches, "user_address").unwrap_or(config.keypair.pubkey());
